# StoryArt: EnhancedEpisodeContext JSON Specification

> **DEPRECATION NOTICE (January 2026)**
>
> **The Redis-based data transfer mechanism described in this document is DEPRECATED.**
>
> StoryArt now retrieves episode context directly from the StoryTeller API:
> - `GET /api/v2/storyart/episode-context/{story_id}/{episode_number}`
> - `POST /api/v1/scene-context/extract-episode-context`
>
> **Redis is no longer required as an intermediary.** The narrative and roadmap data
> are stored in the StoryTeller PostgreSQL database and served via API.
>
> **However, the JSON structure specification (EnhancedEpisodeContext) documented below
> remains the authoritative contract between StoryTeller and StoryArt.** All API
> responses must conform to this structure.
>
> See also:
> - StoryTeller: `app_v2/services/storyart_context_service.py` (builds this structure)
> - StoryTeller: `app_v2/api/storyart_queue.py` (API endpoints)
...
This document provides the **definitive specification for the `EnhancedEpisodeContext` JSON structure** required by StoryArt for image generation. This structure is the contract between StoryTeller and StoryArt.

**Current Data Flow (API-based):**
```
StoryTeller Database --> StoryTeller API --> StoryArt --> SwarmUI
```

StoryArt's services (`contextService.ts`, prompt generation pipeline) are designed to consume this precise format. Adherence to this specification is critical for the entire script-to-image pipeline to function correctly.

---

## 2. Redis Key Convention (DEPRECATED)

> **DEPRECATED**: This section is preserved for historical reference only.
> StoryArt no longer reads from Redis. Use the StoryTeller API instead.

~~All episode data must be stored in Redis using the following key format:~~

~~`session:storyart:episode:{episode_number}`~~

**Current Method:** Call StoryTeller API:
```
GET /api/v2/storyart/episode-context/{story_id}/{episode_number}
```
...
The API response contains an `EnhancedEpisodeContext` object that encapsulates all the narrative, character, and environmental details required for an entire episode.

**This is the authoritative contract between StoryTeller and StoryArt.**
...
This example demonstrates the full structure for a single episode as returned by the StoryTeller API.
